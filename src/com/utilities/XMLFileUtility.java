...
public class XMLFileUtility {
	...
	private void createRootElement() {
		try {
			...
			Element rootNode = new Element("suite").setAttribute("name", GlobalParam.APP_NAME);
//			if (GlobalParam.CURRENT_EXECUTION_MODE.equals("DesktopWeb")) {
				rootNode.setAttribute("parallel", "tests").setAttribute("thread-count", GlobalParam.threadCount);
//			}
	...
	public void createTestNGFile() {
		String parent = System.getProperty("user.dir") + GlobalParam.slash + "TestRepository";
		String child = "testNG-xml";
		File testNGDir = new File(parent, child);
		testNGDir.mkdir();
		String fileName = testNGDir.getAbsolutePath() + GlobalParam.slash + "testNG_" + GlobalParam.FEATURE_TYPE + "_"
				+ getTime() + ".xml";
		try {
			...
			System.out.println("Unable to create testNG file due to: " + e.getMessage());
		}
	}
	...
	private static void modifyTestng(String xmlpath, String value) {
		try {
			...
			rootNode.addContent(new Element("test").setAttribute("name", value).addContent(new Element("classes")
					.addContent(new Element("class").setAttribute("path", "applicationComponent." + value))));
	...
	private Element findElement(Element node, String nodeName, String attribValue, String value) {
	...
	private boolean checkElementPresent(Element mainElement, String searchElementName) {
	...
	private void modifySuite(String testCaseName, String testComponent, Method m) {
		try {
			...
			Element element = findElement(rootNode, "test", "name", testCaseName);
			...
			if (!checkElementPresent(element, "method")) {
				element.addContent(
						new Element("method").addContent(new Element("include").setAttribute("name", m.getName())));
			} else {
				element = element.getChild("method");
				element = element.addContent(new Element("include").setAttribute("name", m.getName()));
			}
			XMLOutputter xmlOutput = new XMLOutputter();
			xmlOutput.setFormat(Format.getPrettyFormat());
			xmlOutput.output(doc, new FileWriter(xmlFile));
	...
	public void modifySuite(String testCaseName, String testComponent, ArrayList<String> methods,
		...
		try {
			...
			for (String browser : browsers) {
				Element element = new Element("test").setAttribute("name",
						String.format("%03d", testCaseNumber) + "_" + testCaseName);
				testCaseNumber++;

				rootNode.addContent(element);
				element.addContent(new Element("parameters"));
				Element parameters = element.getChild("parameters");
				parameters.addContent(new Element("parameter").setAttribute("name", "browser").setAttribute("value",
						browser.toLowerCase()));

				element.addContent(
						new Element("classes").addContent(new Element("class").setAttribute("name", testComponent)));
				...
				for (String method : methods) {
					if (!checkElementPresent(element, "methods")) {
						element.addContent(
								new Element("methods").addContent(new Element("include").setAttribute("name", method)));
					} else {
						Element function = element.getChild("methods");
						function.addContent(new Element("include").setAttribute("name", method));
					}
				}
			}
	...
	private void createTestElement(String testCaseName, String testComponent) {
		try {
			...
			Document doc = (Document) builder.build(xmlFile);
			Element rootNode = doc.getRootElement();
			rootNode.addContent(new Element("test").setAttribute("name", testCaseName).addContent(
					new Element("classes").addContent(new Element("class").setAttribute("name", testComponent))));
