...
public class CommonAll {
	...
	public void openBrowser(String url) throws Exception { // ##U
		try {
			...
			if (GlobalParam.mergeCase) {
				...
				try {
					if (driver.getCurrentUrl().toLowerCase().contains(newURL.toLowerCase())) {
						if (verifyPageInValid())
							reLaunchBrowser();
						else
							return;
					} else {
						if (!blankURLs.contains(driver.getCurrentUrl().toLowerCase()))
							reLaunchBrowser();
					}
				} catch (Exception e) {
					reLaunchBrowser();
			...
			if (!GlobalParam.CURRENT_EXECUTION_MODE.equals("androidWeb")) {
				try {
					...
					System.out.println("get Platform" + actualCapabilities.getPlatform().name());
					org.openqa.selenium.Dimension dim;
					if (actualCapabilities.getPlatform().name().equalsIgnoreCase("mac"))
			...
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Browser opened", "PASS",
					"Browser should be Open");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Browser not opened", "FAIL",
					"Browser should be Open");
	...
	public boolean verifyPageInValid() { // ##U
		try {
			...
			if (src.contains("Problem loading page") || src.contains("This webpage is not available")
					|| src.contains("Response denied by WatchGuard HTTP Proxy")
					|| src.contains("http://images.shopclues.com/images/msg_down.png")
					|| src.contains("You don't have permission to access the page.") || src.isEmpty()
	...
	public void reLaunchBrowser() { // ##U
		...
		Keywords.driver.set(df.getDriver());
	}
		...
		try {
			if (DriverSession.getStepResult()) {
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"Logout button clicked successfully", "PASS", "Logout button should be clicked successfully");
			} else {
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"Logout button not clicked successfully", "FAIL",
	...
	private void typeUserName() throws Exception { // ##U
		...
		try {
			Keywords.typeText(element, DriverSession.getTestCaseData().get("User Name"));
			if (DriverSession.getStepResult()) {
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"User name typed succefully. :: " + DriverSession.getTestCaseData().get("User Name"), "PASS",
						"User name should be entered.");
			} else {
				DriverSession.getLastExecutionReportingInstance().teststepreporting("User name not typed.", "FAIL",
	...
	private void typePassword() throws Exception { // ##U
		...
		try {
			...
			if (DriverSession.getStepResult()) {
				DriverSession.getLastExecutionReportingInstance().teststepreporting("Password typed succefully", "PASS",
						"Password should be entered");
			} else {
				DriverSession.getLastExecutionReportingInstance().teststepreporting("Password not typed.", "FAIL",
	...
	private void clickOnLoginButton() throws Exception { // ##U
		...
		try {
			...
			if (DriverSession.getStepResult()) {
				DriverSession.getLastExecutionReportingInstance().teststepreporting("Clicked on Login Button", "PASS",
						"Should be Clicked on Login Button");
			} else
				DriverSession.getLastExecutionReportingInstance().teststepreporting("Not clicked On Login Button",
						"FAIL", "Should be Clicked on Login Button");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting(
					"Not clicked On Login Button:: " + e.getMessage(), "FAIL", "Should be Clicked on Login Button");
			throw e;
		}
	}
		...
		try {
			...
			if (element.isDisplayed()) {
				...
				if (text.contains("invalid")) {
					DriverSession.getLastExecutionReportingInstance().teststepreporting(text, "FAIL",
							"Error message should not show");
					GlobalParam.ExitTestCase = true;
					throw new Exception("The username or password you have entered is invalid");
				}
			} else {
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"The username or password you have entered is valid.", "PASS",
						"Should be provide valid credentials.");
			}
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Exception...", "FAIL",
					"Error message should not show");
		}
	}
	...
	private void verifyUserIDOnHomePage() throws Exception { // ##U
		...
		try {
			...
			if (text.equalsIgnoreCase(DriverSession.getTestCaseData().get("User Name"))) {
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"User ID on Home page is verified successfully. :: " + text + "=="
								+ DriverSession.getTestCaseData().get("User Name"),
						"PASS", "User ID on Home page should be verified successfully.");
			} else {
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"User ID on Home page is not verified successfully" + text + "!="
								+ DriverSession.getTestCaseData().get("User Name"),
						"FAIL", "User ID on Home page should be verified successfully.");
			}
		} catch (Exception e) {
			e.printStackTrace();
			DriverSession.getLastExecutionReportingInstance().teststepreporting(
					"User ID on Home page is not verified successfully : ", "FAIL",
	...
	public void verifyTextOnPage(String message) { // ##U
		try {
			...
			if (present) {
				DriverSession.getLastExecutionReportingInstance().teststepreporting(message + " is present on page",
						"PASS", "Text should be present on page");
			} else {
				DriverSession.getLastExecutionReportingInstance().teststepreporting(message + " is not present on page",
	...
	public void openURL(String url) { // ##U
		try {
			driver.navigate().to(url);
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Successfully Navigate to Url : " + url,
					"PASS", "Should be able to navigate to specified URL");
		} catch (Exception e) {
			e.printStackTrace();
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Unable to Navigate to Url : " + url,
					"FAIL", "Should be able to navigate to specified URL");
		}
	}

	public void onlyInfo(String message) {
		DriverSession.getLastExecutionReportingInstance().teststepreporting(message, "INFO", "Only Information");
	}
	...
	public void writeUserData(String emailRandom) { // ##U
		try {
			...
			String userRegisterdFilePath = GlobalParam.CURRENT_PROJECT_PATH + "/TestRepository/" + GlobalParam.APP_NAME
					+ "/UserRegistration/userRegistrationDetails" + getTime() + ".txt";
			...
			System.out.println("Text file created successfully at " + file.getCanonicalPath());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void navigateToDesiredPage(String url) {
		try {
			...
			driver.navigate().to(url);
			Thread.sleep(1500);
			DriverSession.getLastExecutionReportingInstance().teststepreporting("Page Navigated successfully", "PASS",
					"page should be navigated to desired url");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting("page not navigated", "FAIL",
					"page should be navigated to desired url");
		}
	}
	...
	protected boolean verifyCartTotalEligibleForGC() throws Exception { // ##U
		...
		try {
			...
			if (total <= cartTotalEligible) {
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"Successfully verify Cart Total on payment page is eligible for apply GC", "PASS",
						"Should be able to verify Cart Total For GC applicable on Payment page");
				return true;
			} else
				DriverSession.getLastExecutionReportingInstance().teststepreporting(
						"GC is not applicable for Cart Total Amout on payment page.", "FAIL",
						"Should be able to verify Cart Total For GC applicable on Payment page");
		} catch (Exception e) {
			DriverSession.getLastExecutionReportingInstance().teststepreporting(
					"Exception ..verify Cart Total on payment page is eligible for apply GC.", "FAIL",
					"Should be able to verify Cart Total For GC applicable on Payment page");
		}
		return false;
	}

	protected boolean verifyCartTotalEligibleForGC(int maxValue) throws Exception { // ##U
		int total;
		try {
			WebElement element = null;
			if (GlobalParam.CURRENT_EXECUTION_MODE.equals("DesktopWeb"))
	...
	public void undoTestChanges() throws IOException { // ##U
		try {
			...
			if (!filepath.isEmpty()) {
				File destDir = new File(GlobalParam.CURRENT_PROJECT_PATH + "/TestRepository/" + GlobalParam.APP_NAME
						+ "/UserRegistration");
	...
	private File getRandomRegistrationFile() throws IOException { // ##U
		...
		int value;
		File file;
		String path = GlobalParam.CURRENT_PROJECT_PATH + "/TestRepository/" + GlobalParam.APP_NAME
				+ "/UserRegistration";
		synchronized (this) {
			do {
			...
			} while ((files[value].isDirectory() && files.length > 1) || files[value].getName().contains(".DS_Store"));
			System.out.println("UserRegistration FileName : " + files[value].getName());
